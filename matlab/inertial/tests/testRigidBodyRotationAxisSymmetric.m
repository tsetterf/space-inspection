...
% Test parameters
ratiosSph = [1.8534 1]';                                % (AS1) SPHERES inertia ratios for CL_MAJ test
omegaB0_B = [1 0.4 0.4]';                               % (AS1)
% ratiosSph = [1.6839 1.6839]';                           % (AS3) SPHERES inertia ratios for CL_MIN test
% omegaB0_B = [0.4 0.4 1]';                             % (AS3)
...
rbrLE        = RigidBodyRotation(ratiosSph,RB0toW,omegaB0_B,'omega0',0);
...
omegaB0_BLEP = [norm(omegaB0_B) 0 0]';
...
% rigidBodyRotation = rbrHE; % Doesn't exist for AS case
