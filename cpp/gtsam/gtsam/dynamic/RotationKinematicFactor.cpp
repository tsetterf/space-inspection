...
RotationKinematicFactor::RotationKinematicFactor(Key PWtoBi, Key PGitoBi,
    Key PWtoBj, Key PGjtoBj, Key tGtoBt_G, const SharedNoiseModel& model)
    : Base(model, PWtoBi, PGitoBi, PWtoBj, PGjtoBj, tGtoBt_G) {

}

/* ************************************************************************* */
Vector RotationKinematicFactor::evaluateError(const Pose3& PWtoBi,
    const Pose3& PGitoBi, const Pose3& PWtoBj, const Pose3& PGjtoBj,
    const Point3& tGtoBt_G, boost::optional<Matrix&> H1,
  ...
  if (H1) {         // dKappa/dRBitoW, and dKappa/tWtoBi_W
    *H1 = (Matrix(3,6) <<
        -skewSymmetric(xi.x(),xi.y(),xi.z()) * RBitoW.matrix() ,
        -RBitoW.matrix()
    ).finished();
  }
  ...
  if (H3) {         // dKappa/dRBjtoW, and dKappa/tWtoBj_W
    *H3 = (Matrix(3,6) <<
        -skewSymmetric(xj.x(),xj.y(),xj.z()) * RBjtoW.matrix() ,
        RBjtoW.matrix()
    ).finished();
  }
  ...
  if (H5) {         // dKappa/tGtoBt_G
    *H5 = (Matrix(3,3) <<
        RBjtoW.matrix()*RGjtoBj.matrix() - RBitoW.matrix()*RGitoBi.matrix()
    ).finished();
  }
